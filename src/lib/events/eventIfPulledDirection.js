...
const id = "EVENT_IF_PULLED_DIRECTION";
...
const autoLabel = (fetchArg) => {
  ...
  if (frames && frames > 0) {
    return l10n("EVENT_IF_PULLED_DIRECTION_FOR_LABEL", {
      direction: fetchArg("direction"),
      frames: String(frames),
    });
  }
  return l10n("EVENT_IF_PULLED_DIRECTION_LABEL", {
    direction: fetchArg("direction"),
  });
};
...
const compile = (input, helpers) => {
  const { ifPulledDirection } = helpers;
...
module.exports = {
  id,
  description: l10n("EVENT_IF_PULLED_DIRECTION_DESC"),
  autoLabel,
  groups,
  subGroups,
  fields,
